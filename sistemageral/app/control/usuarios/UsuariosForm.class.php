...
class UsuariosForm extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        $id = new TEntry('id');
        $empresas_id = new TDBUniqueSearch('empresas_id', 'sistemageral', 'Empresas', 'id', 'empresa_razao_social');
        $empresas_id->setMinLength(0);
        $usuariosniveis_id = new TDBUniqueSearch('usuariosniveis_id', 'sistemageral', 'Usuariosniveis', 'id', 'usuarionivel_nome');
        $usuariosniveis_id->setMinLength(0);
        $usuariossituacoes_id = new TDBUniqueSearch('usuariossituacoes_id', 'sistemageral', 'Usuariossituacoes', 'id', 'usuariossituacao_nome');
        $usuariossituacoes_id->setMinLength(0);
        $usuariostipos_id = new TDBUniqueSearch('usuariostipos_id', 'sistemageral', 'Usuariostipos', 'id', 'usuariostipo_nome');
        ...
        //$usuario_rg->setMask('##.###.###-##');
        $usuario_rg_emissor = new TEntry('usuario_rg_emissor');
        ...
        $this->form->addFields([new TLabel('Id'), $id], [new TLabel('Empresa'), $empresas_id]);
        ...
        $this->form->addFields([new TLabel('Email'), $usuario_email]);
        $this->form->addFields([new TLabel('Senha'), $usuario_senha]);
        //$this->form->addFields([new TLabel('Adicionado Data'), $usuario_adicionado_data]);
        $this->form->addFields(
            [new TLabel('Adicionado Data'), $usuario_adicionado_data],
            [new TLabel('CPF'), $usuario_cpf],
            [new TLabel('RG'), $usuario_rg],
            [new TLabel('Emissor'), $usuario_rg_emissor]
        );
        ...
        $this->form->addFields([new TLabel('Endereco'), $usuario_endereco]);
        $this->form->addFields([new TLabel('Número'), $usuario_numero]);
        $this->form->addFields([new TLabel('Bairro'), $usuario_bairro]);
        $this->form->addFields([new TLabel('Cidade'), $usuario_cidade]);
        $this->form->addFields([new TLabel('Estado'), $usuario_estado]);
        $this->form->addFields([new TLabel('Pais'), $usuario_pais]);
        $this->form->addFields([new TLabel('CEP'), $usuario_cep]);

        $this->form->appendPage('Contatos');

        $this->form->addFields(
            [new TLabel('Fone Residencial'), $usuario_fone1],
            [new TLabel('Celular'), $usuario_fone2]
        );

        $this->form->addFields(
            [new TLabel('Fone de Contato 1'), $usuario_fone3],
            [new TLabel('Fone de Contato 2'), $usuario_fone4]
        );

        $this->form->addFields([new TLabel('Observações'), $usuario_obs]);
        ...
        $usuario_rg_emissor->setSize('100%');
